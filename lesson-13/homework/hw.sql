...
SELECT first_name AS "First Name", LENGTH(first_name)  AS "Name_Length"
FROM employees
...
SUBSTRING(fullname, CHARINDEX(' ', Fullname), charindex(' ',fullname,charindex(' ', fullname)+1)charindex(' ', Fullname)) 
...
SELECT key_column, GROUP_CONCAT(value SEPARATOR ', ') AS concatenated_values
FROM DMLTable
GROUP BY key_column;
...
SELECT department_id,
       ...
       ROUND(100.0 * SUM(CASE WHEN DATEDIFF(CURDATE(), hire_date) > 365*3 THEN 1 ELSE 0 END) / COUNT(*), 2) AS pct_more_than_3_years
FROM employees
...
WITH Ranked AS (
  SELECT
    ...
    job_description,
    years_experience,
    ROW_NUMBER() OVER (PARTITION BY job_description ORDER BY years_experience DESC) AS rn_desc,
    ROW_NUMBER() OVER (PARTITION BY job_description ORDER BY years_experience ASC) AS rn_asc
  FROM Personal
)
SELECT
  job_description,
  MAX(CASE WHEN rn_desc = 1 THEN spaceman_id END) AS most_experienced_id,
  MAX(CASE WHEN rn_asc = 1 THEN spaceman_id END) AS least_experienced_id
FROM Ranked
...
WITH chars AS (
  ...
  FROM (SELECT 1 AS n UNION ALL SELECT 2 UNION ALL SELECT 3 UNION ALL SELECT 4 UNION ALL SELECT 5 UNION ALL SELECT 6 UNION ALL SELECT 7 UNION ALL SELECT 8 UNION ALL SELECT 9 UNION ALL SELECT 10) nums
)
SELECT 
  GROUP_CONCAT(CASE WHEN ch REGEXP '[A-Z]' THEN ch END) AS uppercase_letters,
  GROUP_CONCAT(CASE WHEN ch REGEXP '[a-z]' THEN ch END) AS lowercase_letters,
  GROUP_CONCAT(CASE WHEN ch REGEXP '[0-9]' THEN ch END) AS numbers,
  GROUP_CONCAT(CASE WHEN ch NOT REGEXP '[A-Za-z0-9]' THEN ch END) AS other_chars
FROM chars;
...
SELECT equation,
       (SELECT SUM(CAST(val AS UNSIGNED))
        FROM JSON_TABLE(CONCAT('[', REPLACE(equation, '+', ','), ']'),
                        '$[*]' COLUMNS(val VARCHAR(10) PATH '$')) AS vals) AS answer
FROM Equations;
...
SELECT birthday, COUNT(*) AS count_students
FROM Student
GROUP BY birthday
HAVING COUNT(*) > 1;
