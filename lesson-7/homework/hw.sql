...
Select count(employees) fron Department HR.
...
Select 
	  ...
	  SUM(TotalAmount) AS TOTALSALES,
	  COUNT(DISTINCT ProductID) AS UNIQUE_PROD_SOLD
...
Select 
      ...
      MIN(quantity) AS Min_quantitiy,
	  MAX(quantity) AS Max_qauantity
From Orders
