...
SELECT 
    ...
    CONCAT(
        PARSENAME(REPLACE(Vals, ',', '.'), 2), ',',  -- second value
        PARSENAME(REPLACE(Vals, ',', '.'), 3), ',',  -- first value
        RIGHT(Vals, LEN(Vals) - (CHARINDEX(',', Vals, CHARINDEX(',', Vals) + 1)))  -- remaining string
    ) AS SwappedVals
FROM MultipleVals;
